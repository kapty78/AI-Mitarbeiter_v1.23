-- Zuerst die existierende Funktion entfernen
DROP FUNCTION IF EXISTS public.complete_archive_user(UUID);
DROP FUNCTION IF EXISTS public.complete_archive_user(UUID, UUID);

-- Nun eine angepasste Version erstellen, die die tatsächlichen Spalten der Tabellen verwendet
...
BEGIN
  ...
  -- 1. Dynamisch die Spalten der chat_messages ermitteln
  SELECT array_agg(column_name) INTO column_names 
  FROM information_schema.columns 
  WHERE table_schema = 'public' AND table_name = 'chat_messages';
  
  -- Für den Fall, dass bestimmte Spalten fehlen, protokollieren wir das
  log_entries := array_append(log_entries, 'Chat-Nachrichten Spalten: ' || array_to_string(column_names, ', '));
  
  -- 1. Chat-Nachrichten archivieren - Dynamischer Ansatz mit vorhandenen Spalten
  WITH moved_messages AS (
    INSERT INTO archived_chat_messages (
      id, chat_id, user_id, content, role, created_at, 
      -- entferne model und sentfrom falls nicht vorhanden
      metadata, archived_at, archived_by
    )
    SELECT 
      id, chat_id, user_id, content, role, created_at,
