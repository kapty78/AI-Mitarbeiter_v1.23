...
export default function Chat() {
  ...
  // Ref, um die vorherige Chat-ID zu speichern
  ...
  const sendMessageToAPI = async (
    ...
    // === FIX: Definition wieder hinzufügen ===
    ...
    let chatFactsResults: any[] = []; // Deklaration hier oben
    ...
    if (lastUserMessage) {
        ...
        if (selectedKnowledgeBaseId && selectedKnowledgeBaseId.length > 0) { 
            try {
                ...
                for (const kbId of selectedKnowledgeBaseId) {
                    ...
                    console.log(`[DEBUG] Sending request to /api/knowledge/search for KB ${kbId} with token: ${authToken ? 'Token Present (len=' + authToken.length + ')' : 'NO TOKEN'}`); // DEBUG LOG
        ...
        try {
            ...
            console.log(`✅ ${similarMessages.length} ähnliche Nachrichten gefunden und ${chatFactsResults?.length || 0} Chat-Fakten werden verwendet`);
        ...
        // --- NEU: Suche nach Chat-Fakten ---
        try {
          chatFactsResults = await findRelevantChatFacts(userQuery, userId);
        } catch(error) {
          console.error("❌ Fehler bei der Suche nach Chat-Fakten:", error);
        }
        // --- ENDE NEU --- 
    ...
    const finalPrompt = buildPrompt({
      ...
      chatFacts: chatFactsResults, // Korrekte Übergabe
      allMessages: messages,
      modelType: "default", // Standard-Chat
      maxPastMessages: 2, // Reduced, as facts might be more important now
      maxKnowledgeResults: 3,
      maxChatFacts: 3 // Korrekte Übergabe
    });
    ...
    try {
        ...
        if (streaming) {
          ...
          return {
            ...
            model: model,
            finalPrompt: finalPrompt // Wichtig: Gib den finalPrompt zurück, damit er in handleSendMessage verfügbar ist
          };
        } else {
  ...
  const handleTypewriterComplete = (messageId: string) => {
    ...
    if (message) {
      ...
      // Für sehr lange Nachrichten keine Animation wiederholen 
      // sondern sofort als fertig markieren
...
${selectedTask ? selectedTask.system_prompt : "Fahre mit dem Gespräch fort, basierend auf dieser Zusammenfassung."}`,
  ...
  const handleSendMessage = async (
    ...
    try {
      ...
      if (!chatIdToUse) {
        ...
        // Sofort den Titel generieren, nachdem ein neuer Chat erstellt wurde
        console.log("Generating title for new chat...")
        try {
          await generateAndSetChatTitle(chatIdToUse, message)
          console.log("Title generation initiated")
        } catch (titleError) {
      ...
      try {
        ...
        if (response.streamContent) {
          try {
            console.log("🎬 Starte vereinfachte Streaming-Verarbeitung...");
            let collectedContent = "";
            
            // Direkte Verarbeitung des Streams
            const streamResponse = await fetch(`/api/chat?model=${encodeURIComponent(modelToUse)}&chatId=${encodeURIComponent(chatIdToUse)}`, {
              method: "POST",
              headers: {
                "Content-Type": "application/json"
              },
              body: JSON.stringify({
                messages: response.finalPrompt, // Verwende den finalPrompt aus der sendMessageToAPI-Response
                streaming: true
              })
            });
            
            if (!streamResponse.ok) {
              throw new Error(`API responded with ${streamResponse.status}`);
            }
            ...
            // Streaming-Status setzen, um UI-Elemente während des Streamings auszublenden
            setStreamingMessageId(messageIdToUpdate);
            
            // Einfacherer Streaming-Ansatz: Keine Animation, direkte Updates
            collectedContent = ""; // Zurücksetzen, nicht neu deklarieren
            ...
            try {
              while (true) {
                ...
                // Chunk dekodieren und zum Inhalt hinzufügen
                const chunkText = decoder.decode(value, { stream: true });
                
                // Prüfen auf DONE-Marker am Ende
                if (chunkText.includes("[DONE:")) {
                  console.log("Stream-Ende erkannt:", chunkText);
                  // Entfernen des DONE-Markers vom Text
                  const contentText = chunkText.split("[DONE:")[0];
                  if (contentText.trim()) {
                    collectedContent += contentText;
                  }
                  break;
                }
                
                // Zum Inhalt hinzufügen und UI aktualisieren
                collectedContent += chunkText;
                
                // UI mit Volltext aktualisieren, keine Animation für Streaming
                setMessages(prevMessages => {
                  // Prüfen, ob die Nachricht existiert
                  const exists = prevMessages.some(m => m.id === messageIdToUpdate);
                  if (!exists) {
                    console.warn("Nachricht existiert nicht mehr zum Aktualisieren!");
                    return prevMessages;
                  }
              ...
              reader.releaseLock();
              // Streaming-Status zurücksetzen wenn fertig
              setStreamingMessageId(null);
            }
            
            // Nach Ende des Streams: endgültigen Inhalt speichern
            assistantMessage.content = collectedContent;
            
            console.log(`✅ Streaming abgeschlossen, Gesamtlänge: ${assistantMessage.content.length} Zeichen`);
            
            // Speichern der Assistentennachricht in der Datenbank
            await saveAssistantMessage(assistantMessage, chatIdToUse);
            ...
            // Auch bei Fehlern Streaming-Status zurücksetzen
            setStreamingMessageId(null);
          }
          // <<< HIER return einfügen, damit der Fallback nicht mehr ausgeführt wird
          return;
        } else {
    ...
    try {
      ...
      const { error: aiInsertError, data: aiInsertData } = await supabase
        ...
        .insert({
          ...
          chat_id: chatId,
          role: "assistant",
          // Store content without date prefix
          content: assistantMessage.content, // <- Use original content directly
  ...
  const Message = ({
    ...
    // Prüfen, ob diese Nachricht gerade gestreamt wird
    const isStreaming = streamingMessageId === message.id;

    return (
      <div
        className={`fade-check-parent w-full ${
          message.role === "user" ? "bg-[#1e1e1e]" : "bg-transparent"
        }`}
      >
        <div
          className={`message-container relative mx-auto max-w-3xl p-4 ${
          ...
          <div className="flex items-start space-x-4">
            <div className="flex size-8 shrink-0 items-center justify-center rounded-full">
              {message.role === "user" ? (
                ...
                <div className="flex size-full items-center justify-center rounded-full bg-gray-700 text-white">
                  {message.sentfrom ? message.sentfrom.charAt(0).toUpperCase() : "U"}
                </div>
              ) : (
                // Bot Avatar
                <div className="flex size-full items-center justify-center rounded-full bg-blue-600 text-white">
                  <Sparkles size={16} />
                </div>
              )}
            </div>
            ...
            <div className="flex-1 space-y-2 overflow-hidden">
              ...
              <div className="text-sm font-medium">
                ...
                {message.role === "assistant" && !isStreaming && (
                  <span className="ml-2 text-xs text-gray-400">{selectedModel}</span>
                )}
              </div>

              {/* Message Content */}
              <div className="message-content prose prose-invert max-w-none">
                {/* Keine Animation, direkte Anzeige */}
                <div className="message-fade-container">
                  <MessageMarkdown content={formattedContent} />
                </div>
              </div>

              {/* Message Actions - nur anzeigen, wenn die Nachricht nicht gerade gestreamt wird */}
              {message.role === "assistant" && !isStreaming && !isTyping && (
                <div className="message-actions mt-2 flex space-x-2 opacity-0 transition-opacity group-hover:opacity-100">
                  <button
  ...
  const generateAndSetChatTitle = async (
    ...
    try {
      ...
      // Alle verfügbaren Cookies für die Authentifizierung mitschicken
      const response = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          ...
          model: "Fast", // Fast-Modell entspricht GPT-3.5
          streaming: false, // Kein Streaming für direktere Antwort
          // Authentifizierungsinfos aus dem Session-Cookie werden automatisch mitgesendet
          usedForTitleGeneration: true, // Flag zum Erkennen des Titelgenerierungsaufrufs
          profile: user ? { // User-Profildaten für erweiterte Debugging-Zwecke
            user_id: user.id,
            full_name: userFullName || "Unbekannt"
          } : null
        }),
        credentials: 'include' // Wichtig: Cookies für die Authentifizierung mitsenden
      });
      ...
      // Text-Antwort direkt verarbeiten
      const generatedTitle = await response.text();
      
      console.log(`👋 Raw content for title generation:`, generatedTitle);
      ...
      if (generatedTitle) {
        ...
        if (updateError) {
          ...
          console.log(
            `✅ Chat ${chatId} name updated in DB to: "${cleanTitle}"`
          );
          // Update UI State
          setChatSessions(prevSessions =>
            prevSessions.map(chat =>
        ...
        console.warn("⚠️ Keine gültige Antwort für Titelgenerierung erhalten");
  ...
  const handleGenerateSummary = async () => {
    ...
    try {
      // Direkter API-Aufruf mit speziellen Optionen für die Zusammenfassung
      const chatMessages = messages.filter(m => m.role === "user" || m.role === "assistant");
      
      // Erstelle einen speziellen Prompt für die Zusammenfassung
      const summaryPrompt = `
        Erstelle eine kurze, prägnante Zusammenfassung dieses Gesprächs. 
        WICHTIG: Verwende KEINE Markdown-Überschriften (kein #, ## usw.). 
      ...
      // Manueller API-Aufruf, um mehr Kontrolle über das Format zu haben
      const response = await fetch("/api/chat", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        ...
        body: JSON.stringify({
          ...
          stream: false // Wichtig: kein Streaming für diesen Anwendungsfall
        })
      });
      ...
      // Bereinige den Text von möglichen Markdown-Formatierungen
      ...
      let errorMessage = "Bei der Erstellung der Zusammenfassung ist ein Fehler aufgetreten. Bitte versuche es später erneut."
  ...
  // Add state for tracking which chat's project dropdown is open
  const [projectDropdownOpenFor, setProjectDropdownOpenFor] = useState<string | null>(null)
  
  // Close project dropdown when clicking outside
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (projectDropdownOpenFor && !(event.target as Element).closest('.project-dropdown-area')) {
        setProjectDropdownOpenFor(null)
      }
    }
  ...
  }, [projectDropdownOpenFor])

  // Effekt zum Auslösen der Faktenextraktion beim Verlassen eines Chats
  useEffect(() => {
    // Speichere die aktuelle ID für den nächsten Durchlauf
    const previousChatId = prevChatIdRef.current;
    
    // Update die Ref mit der *jetzt* aktuellen ID
    prevChatIdRef.current = currentChatId;

    // Nur ausführen, wenn sich die ID geändert hat UND die *vorherige* ID nicht null war 
    // (d.h. wir haben einen Chat verlassen)
    if (previousChatId && previousChatId !== currentChatId && user?.id) {
      console.log(`[Fact Extract Trigger] Leaving chat ${previousChatId}. Triggering fact extraction.`);
      
      // Workspace ID für die API ermitteln
      // Finde den verlassenen Chat in den Sessions, um die workspace_id zu bekommen
      const leftChatSession = chatSessions.find(chat => chat.id === previousChatId);
      // Verwende "as any" um auf workspace_id zuzugreifen, falls Typ veraltet
      const workspaceIdForApi = (leftChatSession as any)?.workspace_id || rawWorkspaceId || personalWorkspaceId || null;

      // Rufe die neue API-Route auf
      fetch('/api/chat/extract-embed-chat-facts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          chat_id: previousChatId, 
          user_id: user.id,
          workspace_id: workspaceIdForApi
        }),
      })
      .then(response => response.json())
      .then(data => {
        if (data.success) {
          console.log(`[Fact Extract Trigger] API call successful for chat ${previousChatId}. Extracted ${data.extractedFactCount} facts.`);
        } else {
          console.error(`[Fact Extract Trigger] API call failed for chat ${previousChatId}:`, data.error);
        }
      })
      .catch(error => {
        console.error(`[Fact Extract Trigger] Network error calling fact extract API for chat ${previousChatId}:`, error);
      });
    }

    // Keine explizite Cleanup-Funktion mehr für den alten Batch-Embed-Trigger nötig
    // return () => { ... };
  }, [currentChatId, user?.id, chatSessions, rawWorkspaceId, personalWorkspaceId, supabase]); // Abhängigkeiten: currentChatId, user, chatSessions (für workspace_id lookup), workspaceIds

  // --- NEUE FUNKTION zum Suchen von Chat-Fakten ---
  const findRelevantChatFacts = async (query: string, userId: string) => {
    if (!userId || query.trim().length < 3) {
      console.log("[Chat Facts Search] Skipping search due to missing user ID or short query.");
      return []; // Return empty array if no user or query too short
    }

    console.log(`[Chat Facts Search] Searching relevant chat facts for user ${userId} based on query: "${query.substring(0, 50)}..."`);

    try {
      // 1. Generate embedding for the user query
      // We reuse the logic/API endpoint used by createEmbedding for search-only
      const embeddingResponse = await fetch("/api/embeddings", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ input: query }) // Embed the raw query
      });

      if (!embeddingResponse.ok) {
        throw new Error(`Embedding API responded with ${embeddingResponse.status}`);
      }
      const embeddingData = await embeddingResponse.json();
      const queryEmbedding = embeddingData.embedding;

      if (!queryEmbedding) {
        console.error("[Chat Facts Search] Failed to generate query embedding.");
        return [];
      }

      // 2. Call the new RPC function
      const { data: facts, error: rpcError } = await supabase.rpc(
        'search_chat_facts', 
        {
          query_embedding: queryEmbedding,
          p_user_id: userId,
          similarity_threshold: 0.5, // Ggf. anpassen
          max_results: 3 // Ggf. anpassen
        }
      );

      if (rpcError) {
        console.error("[Chat Facts Search] Error calling search_chat_facts RPC:", rpcError);
        return [];
      }

      console.log(`[Chat Facts Search] Found ${facts?.length || 0} relevant chat facts.`);
      
      // Return the facts (content and similarity)
      return facts || [];

    } catch (error) {
      console.error("[Chat Facts Search] Error finding relevant chat facts:", error);
      return [];
    }
  };
  // --- ENDE NEUE FUNKTION ---

  // Zuerst einen neuen Zustand für aktuell streamende Nachrichten hinzufügen
  // (ca. Zeile 400-450 im Bereich der useState-Deklarationen)
      ...
      {showSidebar && (
        ...
        >
          <div className="flex items-center justify-between border-b border-[#333333] p-4">
            ...
            <button
              onClick={() => {
                // Falls ein Chat aktiv ist, extrahiere die Fakten bevor wir zum Dashboard navigieren
                if (currentChatId && user?.id) {
                  console.log(`[Fact Extract Trigger] Navigating to dashboard from chat ${currentChatId}. Triggering fact extraction.`);
                  
                  // Workspace ID für die API ermitteln
                  const workspaceIdForApi = rawWorkspaceId || personalWorkspaceId || null;
                  
                  // Faktenextraktion API aufrufen
                  fetch('/api/chat/extract-embed-chat-facts', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ 
                  ...
                  .then(data => {
                    if (data.success) {
                      console.log(`[Fact Extract Trigger] API call successful for chat ${currentChatId}. Extracted ${data.extractedFactCount} facts.`);
                    } else {
                      console.error(`[Fact Extract Trigger] API call failed for chat ${currentChatId}:`, data.error);
                    }
                  })
                  .catch(error => {
                    console.error(`[Fact Extract Trigger] Network error calling fact extract API for chat ${currentChatId}:`, error);
                  });
                }
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="relative">
                <button
                  ...
                  className={`rounded p-1.5 hover:bg-gray-800 hover:text-white ${projectFilterActive ? 'bg-[#2d2d2d] text-white' : 'text-gray-400'}`}
                ...
                {showProjectFilterDropdown && (
                  <div className="absolute right-0 z-10 mt-2 w-56 rounded-md bg-[#1e1e1e] shadow-lg ring-1 ring-black ring-opacity-5">
                    <div className="py-1">
                      <button
                        onClick={() => setProjectFilter(null)}
                        className="block w-full px-4 py-2 text-left text-sm text-gray-300 hover:bg-[#2d2d2d]"
                      >
                        Show All
                      </button>
                      {projects.map(project => (
                        <button
                          ...
                          className={`block w-full px-4 py-2 text-left text-sm ${
                            activeProjectFilter === project.id ? 'bg-[#2d2d2d] text-white' : 'text-gray-300 hover:bg-[#2d2d2d]'
                          }`}
                        >
                          {project.name}
                        </button>
          ...
          <div className="custom-scrollbar scrollbar-hide grow overflow-y-auto p-3">
            <div className="mb-4">
              <h2 className="mb-2 text-sm font-semibold uppercase text-[#999999]">
                {projectFilterActive && activeProjectFilter 
                  ? `Chats in ${projects.find(p => p.id === activeProjectFilter)?.name || 'Project'}`
                  : "Chats"}
                {projectFilterActive && (
                  <button 
                    onClick={() => setProjectFilter(null)} 
              ...
              <div className="space-y-1">
                {(projectFilterActive 
                  ...
                  >
                    ...
                    <div className="ml-1 flex min-w-[60px] opacity-0 transition-opacity group-hover:opacity-100">
                      ...
                      {!projectFilterActive && (
                        <div className="project-dropdown-area relative">
                          <button
                            onClick={e => {
                              e.stopPropagation()
                              setProjectDropdownOpenFor(
                                projectDropdownOpenFor === chatSession.id ? null : chatSession.id
                              )
                            }}
                            className="p-1 text-[#cccccc] hover:text-white"
                            title="Einem Projekt zuweisen"
                          >
                            <FileText size={14} />
                          </button>
                          
                          {/* Project selection dropdown */}
                          {projectDropdownOpenFor === chatSession.id && (
                            <div 
                              className="absolute right-0 z-50 mt-1 w-48 rounded-md border border-[#333333] bg-[#1e1e1e] py-1 shadow-lg"
                              onClick={e => e.stopPropagation()}
                            >
                              <div className="px-3 py-2 text-xs font-medium text-[#999999]">
                                Projekt wählen:
                              </div>
                              {/* Option to remove from project if already assigned */}
                              {chatSession.project_id && (
                                <>
                                  <button
                                    ...
                                    className="flex w-full items-center px-4 py-2 text-sm text-red-400 hover:bg-[#2d2d2d]"
                                  >
                                    <span className="block truncate">Von Projekt entfernen</span>
                                  </button>
                              ...
                              {projects.length > 0 ? (
                                <div className="max-h-48 overflow-y-auto">
                                  {projects.map(project => (
                                    <button
                                      ...
                                      onClick={e => {
                                        e.stopPropagation()
                                        assignChatToProject(chatSession.id, project.id)
                                        setProjectDropdownOpenFor(null)
                                      }}
                                ...
                                <div className="px-4 py-2 text-xs italic text-[#666666]">
                      ...
                      {projectFilterActive && chatSession.project_id === activeProjectFilter && (
                        <button
                          onClick={e => {
                            e.stopPropagation()
                ...
                {(projectFilterActive 
                  ...
                  <div className="px-3 py-2 text-xs italic text-[#666666]">
                    {projectFilterActive ? "Keine Chats in diesem Projekt" : "Keine freien Chats"}
                  </div>
                )}
              </div>
            ...
            {/* Only show the projects section if no project filter is active */}
            {!projectFilterActive && (
              <div className="mt-6">
                <h2 className="mb-2 text-sm font-semibold uppercase text-[#999999]">
                  Projekte
          ...
          <div className="flex items-center space-x-4">
            ...
            >
              <button
                ...
                className={`flex items-center space-x-2 rounded-lg border border-[#333333] bg-gradient-to-b from-[#252525]/70 to-[#1e1e1e]/70 px-3 py-1.5 text-sm ${
                  selectedTask
              ...
              >
                ...
                {!selectedTask && <ChevronDown size={16} className="transition-all duration-200" />}
              </button>
              ...
              {showModelDropdown && !selectedTask && (
                <div className="absolute right-0 top-full z-10 mt-1 w-48 rounded-lg border border-[#333333] bg-[#1e1e1e] py-1 shadow-lg">
                  ...
                  <button
                    ...
                    className={`flex w-full items-center px-4 py-2 text-sm transition-all duration-200 hover:bg-[#2d2d2d] hover:text-white ${
                      selectedModel === "Basic"
                        ? "bg-gradient-to-r from-[#e91e63]/20 to-[#c2185b]/20 text-[#e91e63]"
            ...
            >
              <div className="flex items-center">
                <button
                  ...
                  className="flex items-center space-x-2 rounded-lg border border-[#333333] bg-gradient-to-b from-[#252525]/70 to-[#1e1e1e]/70 px-3 py-1.5 text-sm text-[#cccccc] transition-all hover:border-[#444444] hover:text-white disabled:cursor-not-allowed disabled:opacity-50"
                >
                  <span>
              ...
              {showTaskDropdown && (
                <div className="absolute right-0 top-full z-10 mt-1 w-64 rounded-lg border border-[#333333] bg-[#1e1e1e] py-1 shadow-lg">
                  ...
                  <button
                    ...
                    className="flex w-full items-center bg-[#1e1e1e] px-4 py-2 text-sm text-[#cccccc] transition-all duration-200 hover:bg-[#2d2d2d] hover:text-white"
                  >
                    <Plus size={16} className="mr-2" />
                    Neuer Task
                  </button>
        ...
        >
          ...
          >
                ...
                .map(message => (
                  ...
                  >
                    ...
                    {editingMessageId === message.id ? (
                      ...
                      <div
                        className={`max-w-[60%] overflow-hidden rounded-lg ${message.role === "user" ? "user-message bg-blue-600 text-white" : "assistant-message bg-zinc-800 text-white"}`}
                      >
            ...
            {!showFakeMessagesForTour && isTyping && (
              <div className="mb-16 flex justify-start pl-4">
                {" "}
                {/* Increased bottom margin from mb-8 to mb-16 */}
          ...
          <div className="w-full max-w-md rounded-lg border border-[#333333] bg-[#1e1e1e] p-6">
            ...
            <div className="mb-4">
              ...
              <div className="max-h-40 space-y-2 overflow-y-auto rounded-lg border border-[#333333] bg-[#2d2d2d] p-2">
                {knowledgeBases
                  .filter((kb: any) => kb.hasAccess)
                  .map((kb: any) => (
                    <div key={kb.id} className="flex items-center">
                      <input
                        type="checkbox"
                        id={`kb-${kb.id}`}
                        checked={newTaskKnowledgeBaseId.includes(kb.id)}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setNewTaskKnowledgeBaseId([...newTaskKnowledgeBaseId, kb.id])
                          } else {
                            setNewTaskKnowledgeBaseId(
                              newTaskKnowledgeBaseId.filter(id => id !== kb.id)
                            )
                          }
                        }}
                ...
                {knowledgeBases.filter((kb: any) => kb.hasAccess).length === 0 && (
