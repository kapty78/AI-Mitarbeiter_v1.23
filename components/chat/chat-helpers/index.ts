...
import { buildPrompt, adaptForGemini } from "@/lib/prompt-builder"
...
export const handleLocalChat = async (
  ...
  const formattedMessages = buildPrompt({
    userInput: payload.chatMessages[payload.chatMessages.length - 1].message.content,
  ...
  console.log("✅ Verwende konsolidierten Prompt-Builder in handleLocalChat");
...
export const handleHostedChat = async (
  ...
  const modelType = provider === "google" ? "gemini" : "default";
  
  const draftMessages = buildPrompt({
    userInput: payload.chatMessages[payload.chatMessages.length - 1].message.content,
    chatId: payload.chatMessages[0]?.message.chat_id || "",
    taskPrompt: payload.chatSettings.prompt,
    userProfile: profile,
  ...
  console.log(`✅ Verwende konsolidierten Prompt-Builder in handleHostedChat (${modelType})`);

  let formattedMessages: any[] = []
  if (provider === "google") {
    formattedMessages = adaptForGemini(draftMessages);
  } else {
