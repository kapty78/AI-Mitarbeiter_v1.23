...
# Dieses Skript wendet die SQL-Funktionen für die Knowledge Base auf die Supabase-Datenbank an

# Setze die Umgebungsvariablen (aus .env oder manuell)
SUPABASE_URL="${NEXT_PUBLIC_SUPABASE_URL}"
SUPABASE_KEY="${SUPABASE_SERVICE_ROLE_KEY}"

if [ -z "$SUPABASE_URL" ] || [ -z "$SUPABASE_KEY" ]; then
  echo "Fehler: SUPABASE_URL und SUPABASE_KEY müssen gesetzt sein"
  echo "Bitte setze die Umgebungsvariablen oder passe dieses Skript an"
  exit 1
fi

# Extrahiere Projekt-ID aus der URL
PROJECT_ID=$(echo $SUPABASE_URL | sed -E 's/.*\/\/([^.]+).*/\1/')
echo "Projekt-ID: $PROJECT_ID"

# Wende die SQL-Funktion an
echo "Wende SQL-Funktionen auf die Datenbank an..."

# 1. Vector-Extension aktivieren (ohne OpenAI)
cat << 'EOF' | curl -X POST "${SUPABASE_URL}/rest/v1/rpc/pgrest" \
...
echo ""
echo "Vector-Extension aktiviert"

# 2. Neue knowledge_facts_with_chunks-Funktion hinzufügen (geändert, um Embedding als Parameter zu akzeptieren)
cat << 'EOF' | curl -X POST "${SUPABASE_URL}/rest/v1/rpc/pgrest" \
  -H "apikey: ${SUPABASE_KEY}" \
  -H "Authorization: Bearer ${SUPABASE_KEY}" \
  -H "Content-Type: application/json" \
...
echo ""
echo "Neue find_knowledge_facts_with_chunks-Funktion erstellt"

# 3. Trigger für source_chunk-Konvertierung erstellen
cat << 'EOF' | curl -X POST "${SUPABASE_URL}/rest/v1/rpc/pgrest" \
  -H "apikey: ${SUPABASE_KEY}" \
  -H "Authorization: Bearer ${SUPABASE_KEY}" \
  -H "Content-Type: application/json" \
...
echo ""
echo "Trigger für source_chunk-Konvertierung erstellt"

echo "Alle SQL-Funktionen wurden erfolgreich angewendet."
echo "Der Chat sollte jetzt Fakten und ihre Quell-Chunks korrekt finden können." 